...
async function processNodes() {
  let nodes = await Node.find({ status: { $regex: "pending:.*" } }).populate(
    "_user"
  );
  for (let node of nodes) {
    ...
    switch (node.status) {
      case "pending:new":
        ...
        // Create disk
        let createDiskRes = await gcloudDisks.createDiskFromSnapshot(
          project,
          zone,
          newDiskName,
          diskSizeGb,
      ...
      case "pending:cloning":
        ...
        let isDiskReady = await gcloudDisks.isDiskReady(
          project,
          zone,
          node.data_disk_name
        );
